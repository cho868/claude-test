...
| 🖼️ 画像変換 | JPEG/PNG/WebP/GIF/BMP/AVIF/**TIFF/HEIC** を相互変換。**PNG出力は完全可逆（劣化なし）**、JPEG/WebPは品質スライダー付き。透過の保持/背景色での塗りつぶし、長辺リサイズ、複数ファイル一括対応。**変換は全てブラウザ内**で行われ画像はサーバーに送信されない（プライバシー安全・サーバー負荷ゼロ）。 |
| 🔤 文字変換 | **装飾フォント**（𝐋𝐢𝐦𝐛𝐮𝐬 𝐂𝐨𝐦𝐩𝐚𝐧𝐲 / ℒ𝒾𝓂𝒷𝓊𝓈 / 𝕃𝕚𝕞𝕓𝕦𝕤 など22種・タップでコピー）、ケース変換(camel/snake/kebab/全角半角)、URL・Base64・HTMLエンコード、JSON整形、行操作(ソート/重複削除/連番)、SHA-1/256/512ハッシュ、カラーコード(HEX/RGB/HSL)、日時⇔UNIX時刻。全てブラウザ内処理。 |
