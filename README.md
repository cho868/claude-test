...
### 📱 PWA & 🔔 通知
- **ホーム画面に追加してアプリのように使える**（`manifest.json` / Service Worker 同梱・ビルド不要）。
  長押しショートカットから「ソシャゲ日課」「日記を書く」へ直行。オフライン時は案内ページを表示。
- **Web Push でリセット前にリマインド**。未完了が残っている時だけ鳴り、終わっていれば黙る。
  - サーバー側で `php artisan push:vapid` → 出た鍵を `.env` に設定
  - cron に毎時 `php artisan routines:remind` を登録（送信履歴を保存しない設計なのでDB書き込みゼロ）
  - ⚠️ **iPhone / iPad は「ホーム画面に追加」した状態でないと購読できません**（iOS 16.4+ の仕様）
...
| 📋 ソシャゲ日課 | ゲームごとに日課/週課/月課を登録してチェック。**リセット時刻をゲーム単位で設定**（朝5時リセット等）できるので、深夜のチェックが日付をまたいで消えない。チェックは**タップ即反映**（ページ遷移なし）。「今日やること」でゲーム横断の残件とリセットまでの残り時間を表示。原神/FGO/ウマ娘などの**テンプレ**をワンタップ投入。**未完了が残っている時だけ、リセット前に通知**（→ 🔔通知）。 |
...
| 🛠️ 管理エリア（管理者のみ） | フロントの `/admin` から、**🩺セットアップ状況**（VAPID鍵・cronの最終実行・購読端末数・動いているコミット・未適用マイグレーション等を**サーバーの実物を見て自動判定**し、未完了には次に打つコマンドを表示）・**セットアップ/セキュリティ チェックリスト（ToDo）**・統計・サーバー状況・**ユーザー管理**・**🤖 Discord Bot設定**（ステータス/機能ON-OFF/文面を再起動なしで編集。キーはサーバー側で秘匿し localhost API を中継）を操作。 |
