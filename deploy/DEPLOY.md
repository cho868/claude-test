...
| 項目 | 内容 | 対策 |
|------|------|------|
| 体験の再認証 | 2GB=**4日ごと** / **4GB=2日ごと**に管理画面で認証。忘れると停止しVPSごと消える | Discord毎日通知 + 死活監視（NOTIFY.md）。`REAUTH_INTERVAL_DAYS` を必ずプランに合わせる |
...
> ⚠️ **4GBプランは再認証が2日ごと**と、2GB(4日ごと)より短い。RAMは増えるが停止リスクは上がるので、`/etc/portal-notify.conf` の `REAUTH_INTERVAL_DAYS="2"` を必ず設定し、毎日通知を確実に受け取ること。
...
| 3 | **手元PCの SSH known_hosts** | 別サーバーなのでホスト鍵が変わり警告が出る。`ssh-keygen -R NEW_IP` と `ssh-keygen -R madgear.sytes.net` で古い鍵を削除 |
| 4 | **~/.ssh/config** | IP直指定していたら `HostName` を `NEW_IP` に更新。作成したユーザーの公開鍵を新サーバーへ再登録（`ssh-copy-id` 等） |
