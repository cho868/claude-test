...
| 項目 | 内容 | 対策 |
|------|------|------|
| 体験の再認証 | ⚠️**2026-07から新仕様: 有効期限24時間・更新できるのは期限12時間前から**（旧: 2GB=4日/4GB=2日） | `REAUTH_INTERVAL_HOURS="24"` `RENEW_WINDOW_HOURS="12"` を設定し、更新のたびに `sudo deploy/renewed.sh` で記録（NOTIFY.md参照）。長期運用はOracle移行を推奨 |
...
> 🕛 **24時間契約の運用フロー**: ①`REAUTH_INTERVAL_HOURS="24"` `RENEW_WINDOW_HOURS="12"` を設定（更新できるのは期限12時間前から）→ ②更新可能時間に入ると `--reauth-remind` が鳴り始める（それ以前は黙る）→ ③更新したら `sudo /var/www/portal/deploy/renewed.sh` で記録。残り3時間で毎時@everyone。詳細は NOTIFY.md。
