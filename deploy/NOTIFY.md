...
> - 対策: `LINE_URGENT_ONLY="1"`（既定）で **LINEは緊急時（@everyone級）と更新リマインドだけ**に絞る。日常の「稼働中✅」はDiscordのみ。これで通常は月数通〜十数通に収まる。
> - どうしても足りなければ LINE公式アカウントの有料プラン、または LINE をやめて Discord のモバイルプッシュ通知に一本化する手もある。
