...
## 9.5 ソシャゲ日課の通知を有効にする（Web Push）

ポータル自体が PWA（ホーム画面に追加できる）になっているので、**アプリを別に作らなくてもスマホに通知が飛ぶ**。
やることは「鍵を作る」「cron を1行足す」「スマホで購読する」の3つだけ。

### ① VAPID鍵を作って .env に入れる
```bash
cd /var/www/portal
php artisan push:vapid          # 表示された2行をコピー
sudo vi .env                    # VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY に貼る
php artisan config:cache
```
- **秘密鍵を作り直すと、登録済みの端末が全部購読し直しになる**。`.env` はバックアップ対象に入れておく
- `VAPID_SUBJECT` は連絡先（`mailto:` か URL）。既定のままで動く
...
```
```cron
# ソシャゲ日課: リセット前に未完了があれば通知（毎時0分）
0 * * * * cd /var/www/portal && /usr/bin/php artisan routines:remind >/dev/null 2>&1
```
- **未完了が無ければ何も送らない**。全部終わっていれば黙る
- 「リセットの何時間前に鳴らすか」はポータルの `ソシャゲ日課 → ⚙️設定` から各自で変更できる
- **送信履歴をDBに残さない設計**（実行時刻そのものが重複よけ）。SDへの書き込みは増えない
- `schedule:run` は入れていない。毎分PHPを起動するとラズパイには重いので、**直接この1行だけ**にしている

### ③ スマホで購読する
1. Safari / Chrome で `https://xxx.ts.net` を開く
2. **共有 →「ホーム画面に追加」**（⚠️ iPhone はこれをしないと通知を登録できない。iOS 16.4+ の仕様）
3. ホーム画面のアイコンから開き直して `ソシャゲ日課 → ⚙️設定 → この端末で通知を受け取る`
4. 「テスト送信」で実際に鳴るか確認する

### 動作確認・切り分け

**まず `/admin` の「🩺 セットアップ状況」を見る。** 鍵が入っているか・cronが実際に動いた最終時刻・
購読している端末数・未適用のマイグレーションを**サーバーの実物を見て判定**して表示する。
未完了の項目には、そこに打つコマンドがそのまま出るので、上から潰していけば終わる。

```bash
# いま何が送られるか（送信せずに内容だけ表示）
php artisan routines:remind --force --dry-run

# 残り時間に関係なく実際に送る
php artisan routines:remind --force
```
- 鳴らない時にまず見るところ: `.env` の鍵 → `config:cache` したか → スマホ側の通知許可 → ホーム画面から開いているか
- 端末側で通知を解除すると、次回送信時にサーバー側の購読も自動で消える（掃除は不要）
